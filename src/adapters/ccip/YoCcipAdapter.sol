...
///           - Each call leaks zero new balance / zero allowance to the router; leftover native fee
///             and pre-existing dust are recoverable by registered YO vaults via `rescue` /
///             `rescueETH` (see {YoAdapterBase}).
...
contract YoCcipAdapter is YoAdapterBase, IYoCcipAdapter {
    ...
    ///          token leg, and forward `fee` as call value; any excess stays recoverable via
    ///          `rescueETH`.
