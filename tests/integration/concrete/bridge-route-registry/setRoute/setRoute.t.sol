...
contract SetRoute_BridgeRouteRegistry_Integration_Concrete_Test is Integration_Test {
    ...
    function test_WhenVaultZero() external whenCallerOwner {
        ...
        routeRegistry.setRoute(address(0), ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT, true);
    }

    function test_WhenAdapterZero() external whenCallerOwner {
        vm.expectRevert(IYoBridgeRouteRegistry.ZeroAddress.selector);
        routeRegistry.setRoute(users.vault, address(0), TOKEN, DEST_ID, RECIPIENT, OUT, true);
    }

    function test_WhenTokenZero() external whenCallerOwner {
        vm.expectRevert(IYoBridgeRouteRegistry.ZeroAddress.selector);
        routeRegistry.setRoute(users.vault, ADAPTER, address(0), DEST_ID, RECIPIENT, OUT, true);
    }
    ...
    function test_GivenAllowedTrue() external whenCallerOwner whenAllAddressesValid {
        ...
        emit IYoBridgeRouteRegistry.RouteSet(users.vault, ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT, true);
        routeRegistry.setRoute(users.vault, ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT, true);

        assertTrue(
            routeRegistry.isRouteAllowed(users.vault, ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT), "route not allowed"
        );
    }
    ...
    function test_GivenAllowedFalse() external whenCallerOwner whenAllAddressesValid {
        ...
        vm.expectEmit(true, true, true, true, address(routeRegistry));
        emit IYoBridgeRouteRegistry.RouteSet(users.vault, ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT, false);
        routeRegistry.setRoute(users.vault, ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT, false);

        assertFalse(
            routeRegistry.isRouteAllowed(users.vault, ADAPTER, TOKEN, DEST_ID, RECIPIENT, OUT), "route still allowed"
        );
    }
}
