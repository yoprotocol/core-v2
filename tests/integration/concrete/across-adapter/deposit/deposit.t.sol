...
contract Deposit_AcrossAdapter_Integration_Concrete_Test is Integration_Test {
    function _params() internal view returns (IYoAcrossAdapter.DepositParams memory p) {
        p = IYoAcrossAdapter.DepositParams({
            recipient: defaults.BRIDGE_RECIPIENT_ADDR(),
    ...
    function test_WhenRouteNotAllowed() external whenInputAmountNonZero {
        IYoAcrossAdapter.DepositParams memory p = _params();
        p.recipient = address(0xDEAD); // not allowlisted
    ...
    function test_WhenValid() external whenInputAmountNonZero {
        ...
        assertEq(mockSpokePool.depositor(), address(users.vault), "depositor not vault");
        ...
        assertEq(mockSpokePool.inputToken(), address(usdc), "inputToken mismatch");
        ...
        assertEq(mockSpokePool.exclusivityParameter(), p.exclusivityParameter, "exclusivityParameter mismatch");
